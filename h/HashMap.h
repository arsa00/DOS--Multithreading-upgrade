...
public:
	...
	int add(VALUE *val, unsigned key)
	{
		if (val == 0)
			return -1;	// ako je value == null, vracamo -1
	...
	int remove(unsigned key)
	{
		...
		if (ptr != 0)
		{
			...
			delete ptr;		// brise samo prostor za element, ne brise vrednost (value) na koji pokazuje, to je na programeru!
			size--;
...
template<class VALUE>
const unsigned HashMap<VALUE>::N = 25;
