...
public:
	...
	int add(IVTEntry *val, unsigned key)
	{
		if (val == 0)
			return -1;	// ako je IVTEntry == null, vracamo -1
	...
	int remove(unsigned key)
	{
		...
		if (ptr != 0)
		{
			...
			delete ptr;		// brise samo prostor za element, ne brise vrednost (IVTEntry) na koji pokazuje, to je na programeru!
			size--;
